...
void hy3_rope_init(hy3_model *m) {
    ...
    m->rope_pos_jump = 0; m->rope_pos_jump_at = 0;
    if ((e = getenv("HY3_POS_JUMP")))    m->rope_pos_jump = atoi(e);
    if ((e = getenv("HY3_POS_JUMP_AT"))) m->rope_pos_jump_at = atoi(e);
...
static void rope(hy3_model *m, float *q, float *k, int pos, int head_dim, int n_heads, int n_kv_heads) {
    ...
    if (pos >= m->rope_pos_jump_at) pos += m->rope_pos_jump;  /* one-gap long-distance probe */
