...
> 改善到 **约 20 ms/token（≈49 tok/s）** —— 这是本文大部分引用的 kernel 级指标,
> 是设备端工作约 10× 的真实提速。**它不是端到端的用户吞吐。** 在 `eval/` 测试套件上
> 实测的端到端速度（真实的数百 token 生成,含采样/detokenize,且 KV cache 随输出增长）
> 是 **约 16–29 tok/s,通常约 20 tok/s**。短的 `-n 40` 式、上下文很小的运行会读到更高的值
> （之前"44 tok/s"的说法就来自这里）,但不具代表性。请把下文每一步的 tok/s 当作
> 重放/短跑指标上的**相对**进展,而非对持续吞吐的承诺。
...
> 一致、可靠的指标是 **graph 重放 ms/token**,从约 217 → **约 20 ms/token**。
> **真实生成上的持续端到端吞吐是 ~16–29 tok/s（通常约 20）** —— 不要把 "44"/"49"
> 当成用户可见速度。
